function parseURL(url) {
  ...
  if (parts.length >= 9) {
      ...
      console.error('URL structure is not valid:', url);
      return null;
  }
}

export async function loadEmployees(region, title, main) {
  ...
  export default async function decorate(block) {
    ...
    const { region, group, title} = parseURL(pagePath);

    if (clinic) {
      const main = document.createElement('main');
      // eslint-disable-next-line
      const animalsReturned = await loadEmployees(region, title, main);
